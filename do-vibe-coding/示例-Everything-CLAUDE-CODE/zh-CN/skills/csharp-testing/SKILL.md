---
name: csharp-testing
description: 使用 xUnit、FluentAssertions、模拟、集成测试和测试组织最佳实践的 C# 和 .NET 测试模式。origin: ECC
---
# C# 测试模式

使用 xUnit、FluentAssertions 和现代测试实践对 .NET 应用程序进行全面的测试模式。
...
### WebApplicationFactory 设置```csharp
...
|---|---|
|测试实施细节|测试行为和结果 |
|共享可变测试状态 |每个测试的新实例（xUnit 通过构造函数执行此操作）|
|异步测试中的“Thread.Sleep” |使用具有超时功能的“Task.Delay”，或轮询助手 |
|断言 `ToString()` 输出 |对类型化属性进行断言 |
|每次测试一个巨大的断言 |每个测试一个逻辑断言 |
|描述实现的测试名称 |按行为命名：`Method_ExpectedResult_WhenCondition` |
|忽略`CancellationToken` |始终通过并验证取消 |
