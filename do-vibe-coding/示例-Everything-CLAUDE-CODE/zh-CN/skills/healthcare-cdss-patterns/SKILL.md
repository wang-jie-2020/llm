...
description: 临床决策支持系统 (CDSS) 开发模式。药物相互作用检查、剂量验证、临床评分（NEWS2、qSOFA）、警报严重性分类以及集成到 EMR 工作流程中。origin: Health1 Super Speciality Hospitals — contributed by Dr. Keyur Patel
version: "1.0.0"
---
# 医疗保健 CDSS 发展模式

用于构建集成到 EMR 工作流程中的临床决策支持系统的模式。 CDSS 模块对于患者安全至关重要——对假阴性零容忍。
...
1. **`checkInteractions(newDrug, currentMeds, allergies)`** — 根据当前药物和已知过敏情况检查新药。返回按严重性排序的“InteractionAlert[]”。使用“DrugInteractionPair”数据模型。
2. **`validateDose(药物、剂量、途径、体重、年龄、肾功能)`** — 根据基于体重、年龄调整和肾脏调整的规则验证处方剂量。返回“剂量验证结果”。3. **`calculateNEWS2(vitals)`** — 来自 `NEWS2Input` 的国家早期预警分数 2。返回包含总分、风险级别和升级指导的“NEWS2Result”。```
EMR UI
  ↓ (user enters data)
...
```
相互作用对必须是**双向**：如果药物 A 与药物 B 相互作用，那么药物 B 也会与药物 A 相互作用。
...
|严重性 |用户界面行为 |需要临床医生采取行动|
|----------|-------------|--------------------------|
|关键|阻止动作。不可驳回的模态。红色的。 |必须记录覆盖原因才能继续 |
|专业|内嵌警告横幅。橙子。 |继续之前必须确认 |
|次要|内嵌信息注释。黄色的。 |只需意识，无需采取行动 |

重要警报绝不能自动关闭或作为 Toast 通知实施。覆盖原因必须存储在审计跟踪中。

### 测试 CDSS（漏报零容忍）```typescript
...
```
通过标准：100%。一次错过的互动就是患者安全事件。

### 反模式

- 使 CDSS 检查可选或可跳过，无需记录原因
- 将交互检查实施为 Toast 通知
- 对药物或临床数据使用“任何”类型
- 硬编码交互对，而不是使用可维护的数据结构
- 默默地捕获 CDSS 引擎中的错误（必须大声地指出故障）
- 当权重不可用时跳过基于权重的验证（必须阻止，而不是通过）

## 示例

### 示例 1：药物相互作用检查```typescript
