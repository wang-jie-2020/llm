...
description: 生产就绪的 Dart 和 Flutter 模式涵盖空安全、不可变状态、异步组合、小部件架构、流行的状态管理框架（BLoC、Riverpod、Provider）、GoRouter 导航、Dio 网络、冻结代码生成和干净的架构。origin: ECC
---
# 飞镖/颤动模式
...
在以下情况下使用此技能：
- 启动新的 Flutter 功能并需要惯用的状态管理、导航或数据访问模式
- 审查或编写 Dart 代码并需要有关空安全、密封类型或异步组合的指导
- 设置新的 Flutter 项目并在 BLoC、Riverpod 或 Provider 之间进行选择
- 实施安全 HTTP 客户端、WebView 集成或本地存储
- 为 Flutter 小部件、Cubits 或 Riverpod 提供程序编写测试
- 使用身份验证防护连接 GoRouter

## 它是如何工作的

该技能提供了按关注点组织的可复制粘贴的 Dart/Flutter 代码模式：
1. **空安全**——避免`!`，更喜欢`?.`/`??`/模式匹配
2. **不可变状态** — 密封类、`freezed`、`copyWith`
3. **异步组合** — 并发 `Future.wait`，`await` 之后安全的 `BuildContext`
4. **Widget 架构** — 提取到类（而不是方法）、`const` 传播、作用域重建
5. **状态管理** — BLoC/Cubit 事件、Riverpod 通知程序和派生提供程序
6. **导航** - GoRouter 通过 `refreshListenable` 进行响应式身份验证防护7. **网络** — 带拦截器的 Dio，带一次性重试保护的令牌刷新
8. **错误处理** — 全局捕获、`ErrorWidget.builder`、crashlytics 接线
9. **测试** — 单元（BLoC 测试）、小部件（ProviderScope 覆盖）、伪造模拟
...
适用于 Dart 和 Flutter 应用程序的实用、可生产的模式。尽可能与库无关，明确覆盖最常见的生态系统包。

---

## 1. 零安全基础知识

### 更喜欢模式而不是 Bang 运算符```dart
...
- [冻结](https://pub.dev/packages/freezed)
- 技能：`flutter-dart-code-review` — 综合审查清单
- 规则：`rules/dart/` — 编码风格、模式、安全性、测试、挂钩